...
class VerifySignatures {
  ...
  async run () {
    ...
    this.verifiedPackages = new Set()
    const mapper = async (edge) => {
      progress.completeWork(1)
      return this.getVerifiedInfo(edge)
    }
    ...
    if (this.npm.config.get('json')) {
      ...
      if (this.verified && !verified) {
        const msg = this.verified === 1 ?
          `${this.verified} package has a ${this.npm.chalk.bold('verified')} registry signature\n` :
          `${this.verified} packages have ${this.npm.chalk.bold('verified')} registry signatures\n`
  ...
  getEdgeSpec (edge) {
    let alias = edge.name
    try {
      alias = npa(edge.spec).subSpec.name
    } catch (_) {
    }
    try {
      return npa(`${alias}@${edge.spec}`)
    } catch (_) {
  ...
  humanOutput (list) {
    const uniquePackages = new Set(Array.from(list, (v) => {
      const nameVersion = this.npm.chalk.red(`${v.name}@${v.version}`)
...
class Audit extends ArboristWorkspaceCmd {
  ...
  async auditSignatures () {
    ...
    log.verbose('loading intalled dependencies')
    const reporter = this.npm.config.get('json') ? 'json' : 'detail'
    const opts = {
      ...this.npm.flatOptions,
      path: this.npm.prefix,
